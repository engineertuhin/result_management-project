...
$query="select * from   Textile ORDER by semester";
...
?>
 ...
 <link rel="stylesheet" href="css/new.css">
                                            <section class="panel b-a" style="margin-top:30px;  min-height: 1700px;">
												<div class="panel-heading b-b"> <span class="badge bg-warning pull-right">10</span> <a href="#" class="font-bold"> Textile Department all Student</a> </div>
...
<table  class="table table-hover " >
  ...
  <tr>
    ...
    $q="select Subject_1 from  Textile where id='$id'";
    $co=$connect -> query($q);
...
Boardresult,  Textile where  Textile.roll=Boardresult.roll and  Textile.semester=Boardresult.semester and  Textile.department=Boardresult.department ";
...
<?php
 ...
 $id=$data['id']; 
 $roll=$data['roll'];
 $q="select roll from Textile_result where sid='$id'";
 $qu=$connect -> query($q);
 $d=$qu -> fetch_assoc();
...
<div class="container-fluid">
  <td><a href="updateTextile.php?id=<?php echo $data['id'] ?>" title="Update result" class='btn btn-success btn-sm '>Update Result</a>
  </div>
<?php else: ?>
 
  <div class="container-fluid">
  <td><a href="addresultTextile.php?id=<?php echo $data['id'] ?>" title="Add result" class='btn <?php echo $color ?> btn-sm <?php echo $masb?>'><?php echo $result?></a>
  </div>
    <?php endif?>
    ...
    <a href="editetextile.php?id=<?php echo $data['id'] ?>" title="Edit" style="margin-top:5px;" class='btn btn-info  btn-sm'><i class="fas fa-user-edit"></i></a>
   
    <a href="deleteTexttile.php?id=<?php echo $data['id'] ?>" title="Delete" style="margin-top:5px;" class='btn btn-danger btn-sm'><i class="fas fa-trash-alt"></i></a>
